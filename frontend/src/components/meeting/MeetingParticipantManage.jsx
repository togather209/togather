...
function MeetingParticipantManage({
  name,
  meetingDetail,
  guestId,
  forR,
  ...
  const joinRequestAccept = async () => {
    try {
      const response = await axiosInstance.post(
        `/teams/${id}/join-requests/${guestId}/accept`
      );
      setForR(!forR);
    } catch (error) {
  ...
  const joinRequestReject = async () => {
    try {
      const response = await axiosInstance.post(
        `/teams/${id}/join-requests/${guestId}/reject`
      );
      setForR(!forR);
    } catch (error) {
      console.error("데이터 불러오기 실패", error);
  ...
  if (joinrequeststatus === 0) {
    return (
      <div className="meeting-participant-request-box">
        ...
        <div>
          <img
            onClick={joinRequestAccept}
            className="meeting-request-icon"
            src={agree}
            alt="수락"
          />
          <img
